...
// AUTHOR: Max Comfort
// FILENAME: Lab5.java
// SPECIFICATION: A program that draws a row, pyramid or diamond
//                that the user chooses and inputs data
// FOR: CSE110 - Lab 5
// TIME SPENT: 8 hours
//-----------------------------------------------------------*/
...
public class Lab5 {
    ...
    private static String buildDiamond(
        ...
        for (int numSpaces = 1; numSpaces < numSymbols; numSpaces += 2) { //build upper part of diamond
            int numStars = (totalSymbOneRow - numSpaces) / 2;
            String row = buildRow(sideSymb, numStars, midSymb, numSpaces);
            result += row + "\n";
        }
        
        for (int numSpaces = numSymbols; numSpaces > 0; numSpaces -= 2) { //build lower part of diamond
            int numStars = (totalSymbOneRow - numSpaces) / 2;
            String row = buildRow(sideSymb, numStars, midSymb, numSpaces);
            result += row + "\n";
        }
